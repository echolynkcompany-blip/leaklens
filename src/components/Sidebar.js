...
export default function Sidebar({ page, setPage, practices, activePracticeId, setActivePracticeId, onAddPractice, user }) {
  ...
  return (
    <aside style={S.sidebar}>
          ...
          {practices.map(p => (
            <button key={p.id} style={{ ...S.practiceBtn, ...(p.id === activePracticeId ? S.practiceBtnActive : {}) }}
              ...
              <div style={{ flex: 1, textAlign: 'left', overflow: 'hidden' }}>
                ...
                <div style={{ fontSize: 10, color: 'var(--text3)' }}>{p.month}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      {/* Nav */}
      <div style={{ ...S.section, flex: 1, borderBottom: 'none' }}>
...
const S = {
  sidebar: { width: 'var(--sidebar)', background: 'var(--bg2)', borderRight: '1px solid var(--border)', position: 'fixed', top: 0, left: 0, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', zIndex: 100 },
  logo: { display: 'flex', alignItems: 'center', gap: 10, padding: '1.25rem 1rem', borderBottom: '1px solid var(--border)', flexShrink: 0 },
  mark: { width: 32, height: 32, borderRadius: 8, background: 'var(--teal)', color: '#0a0c10', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, fontFamily: 'var(--mono)', flexShrink: 0 },
  title: { fontSize: 14, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.3px' },
  sub: { fontSize: 10, color: 'var(--text3)', marginTop: 1 },
  section: { padding: '0.875rem', borderBottom: '1px solid var(--border)', flexShrink: 0 },
  label: { fontSize: 10, fontWeight: 600, color: 'var(--text3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.5rem' },
  practiceBtn: { display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '6px 8px', borderRadius: 7, border: '1px solid transparent', background: 'transparent', cursor: 'pointer', marginBottom: 2 },
  practiceBtnActive: { background: 'var(--bg3)', border: '1px solid var(--border2)' },
  dot: { width: 6, height: 6, borderRadius: '50%', flexShrink: 0 },
  input: { width: '100%', padding: '6px 8px', borderRadius: 6, border: '1px solid var(--border2)', background: 'var(--bg3)', color: 'var(--text)', fontSize: 12, outline: 'none' },
  addBtn: { width: '100%', padding: '6px 8px', borderRadius: 7, border: '1px dashed var(--border2)', background: 'transparent', color: 'var(--text3)', cursor: 'pointer', fontSize: 12, marginTop: 4, textAlign: 'left' },
  addOk: { flex: 1, padding: '5px', borderRadius: 6, border: 'none', background: 'var(--teal)', color: '#0a0c10', fontSize: 12, fontWeight: 600, cursor: 'pointer' },
  addCancel: { flex: 1, padding: '5px', borderRadius: 6, border: '1px solid var(--border2)', background: 'transparent', color: 'var(--text2)', fontSize: 12, cursor: 'pointer' },
  navBtn: { display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '8px 10px', borderRadius: 7, border: 'none', background: 'transparent', color: 'var(--text2)', cursor: 'pointer', fontSize: 13, marginBottom: 1, textAlign: 'left' },
  navActive: { background: 'var(--teal-dim)', color: 'var(--teal)' },
  userRow: { padding: '0.875rem', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 },
  userAvatar: { width: 28, height: 28, borderRadius: '50%', background: 'var(--blue-dim)', color: 'var(--blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, flexShrink: 0 },
  signOut: { padding: '4px 6px', borderRadius: 6, border: '1px solid var(--border2)', background: 'transparent', color: 'var(--text3)', fontSize: 14, cursor: 'pointer' },
};
